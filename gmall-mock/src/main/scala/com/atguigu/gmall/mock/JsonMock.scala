...
object JsonMock {
    ...
    def initOneStartupLog(): String = {
        ...
        `ts` bigint COMMENT '启动时间',    考虑每个终端的时间的不准性, 时间是将来在服务器端来生成.. 不用客户端时间
        `area` string COMMENT '城市'
        `channel` string COMMENT '渠道'
         */
        val mid: String = "mid_" + RandomNumUtil.randomInt(1, 1)
    ...
    def generateLog(): Unit = {
        (0 to startupNum).foreach(_ => {
            ...
            LogUploader.sendLog(oneStartupLog) // 等到服务好了之后,再打开
            // 模拟出来多条事件日志
